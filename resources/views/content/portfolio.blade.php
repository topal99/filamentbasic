        ...
        <div class="relative overflow-hidden cursor-pointer mb-6">
            <a href="{{ Storage::url($portofolio->image) }}" data-gallery="gallery1"
                ...
                <img class="block w-full h-auto transform duration-500 grayscale hover:scale-125"
                    src="{{ Storage::url($portofolio->image) }}" alt="{{ $portofolio->title }}">
                <div
                    ...
                    <h3 class="text-base leading-normal font-semibold my-1 text-white">{{ $portofolio->title }}</h3>
                    <small class="d-block">{{ $portofolio->category->name }}</small>
                </div>
            </a>
        </div>
    </figure>
